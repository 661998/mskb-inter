...
namespace Drupal\part_2_examination_customization\Controller;
...
/**
 * Class ExaminationPart2Pdf.
 */
class ExaminationPart2Pdf extends ControllerBase {
  ...
  public function content($nid) {
    ...
    $field_part_1_admission_challan = $field_part_1_admit_card = $field_part_1_marksheet = $caste_certificate_url = '';
    $field_part_2_admission_challan = $field_part_2_promoted_admit_card = $field_part_2_promoted_marksheet = '';
    if (!empty($node->get('field_part_1_admission_challan')->target_id)) {
        $field_part_1_admission_challan = File::load($node->get('field_part_1_admission_challan')->target_id);
    }
    if (!empty($node->get('field_part_1_admit_card')->target_id)) {
        $field_part_1_admit_card = File::load($node->get('field_part_1_admit_card')->target_id);
    }
    if (!empty($node->get('field_part_1_marksheet')->target_id)) {
        $field_part_1_marksheet = File::load($node->get('field_part_1_marksheet')->target_id);
    }
    if (!empty($node->get('field_part_1_marksheet')->target_id)) {
        $field_part_2_admission_challan = File::load($node->get('field_part_2_admission_challan')->target_id);
    }
    if (!empty($node->get('field_part_1_marksheet')->target_id)) {
        $field_part_2_promoted_admit_card = File::load($node->get('field_part_2_promoted_admit_card')->target_id);
    }
    if (!empty($node->get('field_part_1_marksheet')->target_id)) {
        $field_part_2_promoted_marksheet = File::load($node->get('field_part_2_promoted_marksheet')->target_id);
    }
      ...
      <h2>Part 2 Examination Form</h2>
            ...
            <tr>
                <td style="width: 70%">
                    <table style="width: 100%">
                        ...
                        <tr>
                          <th style="width: 50%; text-align: left">Exam Type</th>
                          <td style="width: 50%">'.$node->get('field_exam_type')->value.'</td>
                        </tr>
                        <tr>
        ...
        <table  style="width: 100%">
            <tr>
                ...
                if(!empty($field_part_1_admission_challan)){
                    $data .= '<th style="width: 20%; border:1px solid black">Part 1 Admission Challan</th>';
                }
                if(!empty($field_part_1_admit_card)){
                    $data .= '<th style="width: 20%; border:1px solid black">Part 1 Admit Card</th>';
                }
                if(!empty($field_part_1_marksheet)){
                    $data .= '<th style="width: 20%; border:1px solid black">Part 1 Marksheet</th>';
                }
                if(!empty($field_part_2_admission_challan)){
                    $data .= '<th style="width: 20%; border:1px solid black">Part 2 Admission Challan</th>';
                }
                if(!empty($field_part_2_promoted_admit_card)){
                    $data .= '<th style="width: 20%; border:1px solid black">Part 2 Promoted Admit Card</th>';
                }
                if(!empty($field_part_2_promoted_marksheet)){
                    $data .= '<th style="width: 20%; border:1px solid black">Part 2 Promoted Marksheet</th>';
                }
            $data .= '</tr>
            <tr>
                ...
                if(!empty($field_part_1_admission_challan)){
                    $data .= '<td style="width: 20%; border:1px solid black; text-align:center"><a href="'.$site.$field_part_1_admission_challan->createFileUrl().'">View Document</a></td>';
                }
                if(!empty($field_part_1_admit_card)){
                    $data .= '<td style="width: 20%; border:1px solid black; text-align:center"><a href="'.$site.$field_part_1_admit_card->createFileUrl().'">View Document</a></td>';
                }
                if(!empty($field_part_1_marksheet)){
                    $data .= '<td style="width: 20%; border:1px solid black; text-align:center"><a href="'.$site.$field_part_1_marksheet->createFileUrl().'">View Document</a></td>';
                }
                if(!empty($field_part_2_admission_challan)){
                    $data .= '<td style="width: 20%; border:1px solid black; text-align:center"><a href="'.$site.$field_part_2_admission_challan->createFileUrl().'">View Document</a></td>';
                }
                if(!empty($field_part_2_promoted_admit_card)){
                    $data .= '<td style="width: 20%; border:1px solid black; text-align:center"><a href="'.$site.$field_part_2_promoted_admit_card->createFileUrl().'">View Document</a></td>';
                }
                if(!empty($field_part_2_promoted_marksheet)){
                    $data .= '<td style="width: 20%; border:1px solid black; text-align:center"><a href="'.$site.$field_part_2_promoted_marksheet->createFileUrl().'">View Document</a></td>';
                }
            $data .= '</tr>
         </table>
    ...
    $stylesheet = file_get_contents('modules/custom/part_2_examination_customization/css/pdf.css');
